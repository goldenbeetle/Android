...
public class Utils {
	public static final String DEVELOPER_NAME = "Golden+Beetle";
	...
	public static void moreAppsFromDeveloper(Context con, String devloperName) {
		try {
		    con.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("market://search?q=pub:" + devloperName)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
		} catch (android.content.ActivityNotFoundException anfe) {
		    try {con.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("http://play.google.com/store/search?q=pub:" + devloperName)).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));} catch(Exception e){}
		}   
	}
		...
		try {
	    	...
	    	if (packagename==null || packagename.equals("") || packagename.equalsIgnoreCase(con.getPackageName())) {
		    	if (urltype!=null && urlpath!=null && !urltype.equalsIgnoreCase("none") && !urlpath.equalsIgnoreCase("")) {
		    		if (urltype.equalsIgnoreCase("youtube")) {
		        		if (!urlpath.equals("")) {
		        			//ViewSongListOnYoutube( new ArrayList<String>(Arrays.asList(urlpath.split(","))),con);
		        		}
		        	} else if (urltype.equalsIgnoreCase("gplay")) {
		        		try {
